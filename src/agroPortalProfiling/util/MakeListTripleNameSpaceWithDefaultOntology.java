...
public class MakeListTripleNameSpaceWithDefaultOntology {
	...
	public static ArrayList<UriAndUriAndUriAndNumber> makeList(Model model) {
		...
		ArrayList<UriAndUriAndUriAndNumber> ListResources = new ArrayList<UriAndUriAndUriAndNumber>();
		 ...
		 String sparqlQuery = prefix +
			...
			" 	     	?s1 ?p1 ?o1 . " +
			"	 		FILTER(STR(?o1) = STR(owl:Ontology))" +
			...
			" 		FILTER(STRSTARTS(STR(?namespaceS), STR(?uriOntology)) || STRSTARTS(STR(?namespaceP), STR(?uriOntology)) || STRSTARTS(STR(?namespaceO), STR(?uriOntology)))" +
            " } " +
