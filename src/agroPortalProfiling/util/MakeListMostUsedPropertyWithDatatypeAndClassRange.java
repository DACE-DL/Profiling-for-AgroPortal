	...
	public static ArrayList<UriAndUriListAndNumberListAndUriListAndNumberListAndNumber> makeList(Model model, ArrayList<UriAndNumber> listMostUsedProperty) {
		...
		for (UriAndNumber uriAndNumber : listMostUsedProperty) {
			...
			if (result.hasNext()) {
				...
				// System.out.println("Running time for" + property + ": " + AgroPortalProfilingUtil.getDurationAsString(Duration.between(start0, end0).toMillis()));
			}
			qe.close(); // Libère la mémoire après utilisation de la requête
		}
